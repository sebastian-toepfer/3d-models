...
for include_dir in as_list(
    env.GetProjectOption("custom_explicit_library_include_dirs", [])
):
    env.Append(CPPPATH=["$PROJECT_DIR/%s" % include_dir])

for src_dir in as_list(env.GetProjectOption("custom_explicit_library_sources", [])):
    build_dir = "$BUILD_DIR/explicit-library-sources/%s" % basename(normpath(src_dir))
